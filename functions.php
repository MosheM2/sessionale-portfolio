...
function portfolio_migration_start_import() {
    ...
    // Increase execution time and memory for import
    ...
    // Use the enhanced import class if available
    $enhanced_file = get_template_directory() . '/portfolio-import-enhanced.php';
    if (file_exists($enhanced_file)) {
        require_once($enhanced_file);
        
        // Fetch the Adobe Portfolio page to get project URLs
        $response = wp_remote_get($url, array('timeout' => 30));
        
        if (is_wp_error($response)) {
            wp_send_json_error(array('message' => __('Could not connect to Adobe Portfolio. Error: ', 'sessionale-portfolio') . $response->get_error_message()));
        }
        
        $body = wp_remote_retrieve_body($response);
        
        if (empty($body)) {
            wp_send_json_error(array('message' => __('No content found at the specified URL.', 'sessionale-portfolio')));
        }
        
        // Extract project URLs from the main page
        $project_urls = portfolio_migration_extract_project_urls($body, $url);
        
        if (empty($project_urls)) {
            wp_send_json_error(array('message' => __('No portfolio projects found at the specified URL.', 'sessionale-portfolio')));
        }
        ...
        foreach ($project_urls as $project_url) {
            $post_id = $importer->import_portfolio_project($project_url, 'portfolio');
            if ($post_id) {
                $imported_count++;
            }
        ...
        if ($imported_count > 0) {
            wp_send_json_success(array(
                'message' => sprintf(_n('%d project imported successfully', '%d projects imported successfully', $imported_count, 'sessionale-portfolio'), $imported_count),
                'projects' => $imported_count,
                'total' => $total
            ));
        } else {
            wp_send_json_error(array('message' => __('No new projects to import. All projects may already exist.', 'sessionale-portfolio')));
        }
        ...
        // Parse the HTML
        $result = portfolio_migration_parse_and_import($body, $url);
        ...
        $messages = array();
        if ($imported_count > 0) {
            $messages[] = sprintf(_n('%d new project imported', '%d new projects imported', $imported_count, 'sessionale-portfolio'), $imported_count);
        }
        if ($images_added > 0) {
            $messages[] = sprintf(_n('%d image added to existing project', '%d images added to existing projects', $images_added, 'sessionale-portfolio'), $images_added);
        }
        
        if ($imported_count > 0 || $images_added > 0) {
            wp_send_json_success(array(
            ...
            wp_send_json_error(array('message' => __('No new projects to import. All existing projects already have images.', 'sessionale-portfolio')));
        }
    }
}
 ...
 */
function portfolio_migration_extract_project_urls($html, $base_url) {
    $project_urls = array();
    ...
    foreach ($links as $link) {
        ...
        if (!in_array($project_url, $project_urls)) {
            ...
            error_log('Portfolio Import: Found project URL: ' . $project_url);
        }
    }
...
/**
 * Parse Adobe Portfolio HTML and Import Projects
 */
function portfolio_migration_parse_and_import($html, $base_url) {
    $imported = 0;
    $images_added = 0;
